...
class Network(object):
    ...
    def _setup_placeholders(self):
        ...
        self.apha = tf.compat.v1.placeholder(tf.float32, [None, self.dia_max_len, self.nb_classes], name="handoff_y")
    ...
    def train_cf(self, data_generator, keep_prob, epochs, data_name,
                 mode='train', batch_size=20, nb_classes=2, shuffle=True,
                 is_val=True, is_test=True, save_best=True,
                 val_mode='eval', test_mode='test'):
        ...
        for epoch in range(epochs):
            ...
            for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
                ...
                try:
                    if epoch >= 20:
                        ...
                        _, step, summaries, loss, sequence, senti, score = self.sess.run(
                            [self.train_op, self.global_step, self.summaries,
                            self.loss_pre, self.output, self.senti, self.pre_score], feed_dict)
                    self.summary_writer_train.add_summary(summaries, step)
                    # for handoff metric
                    handoff_y = np.argmax(batch_handoff, -1)
                    # for senti metric
            ...
            # for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
            #     batch_handoff, batch_senti, batch_score in data_generator(mode='counterfactual', batch_size=batch_size,
            #                                                               nb_classes=nb_classes,
            #                                                               shuffle=shuffle, epoch=epoch):
            #     feed_dict = {self.input_x1: batch_dialog_ids,
            #                  self.role_list: batch_role_ids,
            ...
            #         # _, step, summaries, loss, sequence, senti, score = self.sess.run(
            #         #     [self.train_op, self.global_step, self.summaries,
            #         #      self.loss_cf, self.output, self.senti, self.pre_score], feed_dict)
            #         _, step, summaries, loss, sequence, senti, score = self.sess.run(
            #             [self.train_op, self.global_step, self.summaries,
            #              self.loss_cf, self.output, self.senti, self.pre_score], feed_dict)
            #         self.summary_writer_train.add_summary(summaries, step)
