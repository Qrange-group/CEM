...
def print_trainable_variables(output_detail, logger):
    ...
    if logger:
        ...
        logger.info(
            ...
            % (len(tf.compat.v1.trainable_variables()), "{:,}".format(total_parameters))
        )
    else:
        if output_detail:
            print("\n" + parameters_string)
...
def show_memory_use():
    ...
    total_memory = (
        1.0 * (ru.ru_maxrss + ru.ru_ixrss + ru.ru_idrss + ru.ru_isrss) / rusage_denom
    )
    strinfo = (
        "\x1b[33m [Memory] Total Memory Use: %.4f MB \t Resident: %ld Shared: %ld UnshareData: "
...
def get_gtt_score(label_list, pre_list, lamb=0.0):
    gtt_score_list_1 = []
    gtt_score_list_2 = []
    gtt_score_list_3 = []
    for pres, labels in zip(pre_list, label_list):
        gtt_score_list_1.append(
            golden_transfer_within_tolerance_exp(pres, labels, t=1, lamb=lamb)
        )
        gtt_score_list_2.append(
            golden_transfer_within_tolerance_exp(pres, labels, t=2, lamb=lamb)
        )
        gtt_score_list_3.append(
            golden_transfer_within_tolerance_exp(pres, labels, t=3, lamb=lamb)
        )

    return (
        np.mean(gtt_score_list_1),
        np.mean(gtt_score_list_2),
        np.mean(gtt_score_list_3),
    )
