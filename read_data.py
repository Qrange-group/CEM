...
def read(data_name, mode):
    ...
    with open(load_path, 'rb') as fin:
        # X
        dialogues_ids_list = pkl.load(fin)        # (?, dia_len, sent_len) (?, ?, 64) 对话长度最长为128. 每一行表示一个对话，一个对话用多条list表示，每一条list表示对话中的一句话
        dialogues_sent_len_list = pkl.load(fin)   # 记录一个对话的每句话长度，一个对话用一条list表示，list长度与dialogues_ids_list的list数量对应，每一个数值表示对应的dialogues_ids_list的list的非0数目
...
def create_cf_data(data_name, mode):
    ...
    with open(load_path, 'rb') as fin:
        ...
        data = data.sort_values('dialogues_len_list')[int(len(dialogues_len_list)*0.8):].reset_index(drop=True)
