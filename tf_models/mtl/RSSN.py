...
class RSSN(Network):
    ...
    def _inference(self):
        ...
        with tf.name_scope('utterance_encoder'):
            ...
            self.sent_encoder_attened_concat = tf.concat([self.utter_attened, self.utter_encode_state], axis=-1)
            ...
            self.sent_encoder_attened_intersection = tf.layers.dense(self.sent_encoder_attened_concat, self.rnn_dim, activation=tf.nn.tanh, use_bias=True) 
            self.sent_encoder_attened_reshape = tf.reshape(tf.concat([self.sent_encoder_attened_intersection, self.sent_encoder_attened_concat], axis=-1), shape=[-1, self.dia_max_len, 5 * self.rnn_dim])
            ...
            if self.dropout_keep_prob != 1:
                # [B * T, 2 * H]
                self.sent_encoder_attened_reshape = tf.nn.dropout(self.sent_encoder_attened_reshape,
        ...
        with tf.name_scope('dialogue_mask'):
            ...
            self.agent_mask = tf.tile(tf.expand_dims(tf.add(tf.negative(self.role_list), 1), axis=-1), multiples=[1, 1, self.dia_max_len])
        ...
        with tf.name_scope('universal_context'):
            # match context
            self.cross_match = tf.matmul(self.sent_encoder_attened_reshape,
            ...
            self.encode_match = tf.concat([self.sent_encoder_attened_reshape, self.cross_match_sim_one_direct], axis=-1)
        ...
        with tf.name_scope('ssa2mhch_context'):
            ...
            self.ssa2mhch_final_vec = tf.keras.layers.Dense(units=self.dense_dim, activation='relu')(self.ssa2mhch_concat)
        ...
        with tf.name_scope('ssa_dis_combination'):
            ...
            self.ssa_combine_vec = tf.reduce_sum(self.ssa_ff_customer_distri * tf.expand_dims(self.weights_local, -1), 1)
        ...
        with tf.name_scope("output"):
            ...
            self.main_logits = self.main_logits * tf.expand_dims(tf.concat([tf.expand_dims(self.score_logits[:, 0], axis=-1), tf.expand_dims(self.score_logits[:, 2], axis=-1)], -1), 1)
    ...
    def _compute_loss(self):
        ...
        # Combine loss
        self.loss = self.main_loss + self.loss_lambda * self.score_loss + 0.01 * self.cost_loss_simulator
        self.loss_pre = tf.reduce_mean(tf.square(self.main_logits[:, 1] - self.main_y[:, 1]))
