...
class PPO(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(self.embedding_dim, self.embedding_dim, 6)

        self.w1 = nn.Linear(self.time_step * self.sent_max_len * self.embedding_dim, 256)
        self.w_pi = nn.Linear(256, 2)
        self.w_loss = nn.CrossEntropyLoss()
        # self.w_loss = nn.BCEWithLogitsLoss(reduction='mean')
        # self.w_loss = nn.L1Loss(reduction='mean')

        self.optimizer = optim.Adam(self.parameters(), lr=learning_rate)

    def word_pi(self, x, softmax_dim = 0):
        prob = F.softmax(self.w_pi(F.relu(self.w1(x))), dim=softmax_dim)
        return prob
    ...
    def pretrain_word_embedding(self, ifTestTrain=False, ifTest=False):
        ...
        for j in range(batch_num):
            ...
            if index.shape[0] != 0:
                ...
                pi = self.word_pi(s_batch, softmax_dim=1)
                # pi_a = pi.gather(1, a_batch)
                # _, pi_a = torch.max(pi, 1)
                # a_batch = a_batch.float()
                # a_batch.requires_grad_(requires_grad=True)
                # a_batch.retain_grad()
                #print('~~~~',torch.sum(a_batch)/1024)
                #print('???',a_batch.size())
                #print('!!!',pi.size())
                a_batch = a_batch.squeeze()
                leng = a_batch.size(0)
                a_batch = torch.empty(leng, dtype=torch.long).random_(2).to(device)
                #print('sdf',a_batch.size())


	
                loss_classify = self.w_loss(pi, a_batch)
                #print(loss_classify)
                # print(a_batch)
                # print(a_batch.shape)
                print(loss_classify)
                ...
                self.optimizer.step()
                print(self.w1.weight.grad)
                print(self.w1.weight)
                self.optimizer.zero_grad()
                loss_sum += loss_classify.item()
        ...
        if ifTestTrain:
            self.w_simple_evaluate_batch(mode='train')     
        if ifTest:
            self.w_simple_evaluate_batch(mode='test')

        return loss_sum

    def w_simple_evaluate_batch(self, mode='test'):
    ...
    def train_net(self, ifTestTrain=False, ifTest=False):
        ...
        if ifTestTrain:
            self.simple_evaluate_batch(mode='train')     
        if ifTest:
            self.evaluate_batch()
...
def main():
    ...
    model = PPO().to(device)
    #print(model.w1.weight.grad,'??????before')
    ...
    for n_epi in range(10000):
        if n_epi < 100:
            loss = model.pretrain_word_embedding(ifTestTrain = True, ifTest = True)
            #print(model.w1.weight,n_epi)
            #print(model.w1.weight.grad,'~~~after',n_epi)
            #print('@@@@@@',torch.sum(model.w_pi.weight.grad))
            #print('#$@#@$',torch.sum(model.fc1.weight.grad))
            #print('!!!!!!!!!!!!!!!!!',torch.sum(model.w1.weight.grad))
        else:
