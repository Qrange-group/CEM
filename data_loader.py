...
class DataLoader(object):
            ...
            with open(load_path, 'rb') as fin:
                ...
                if mode == 'counterfactual':
                    ...
                    for i in range(len(self.handoff_list)):
                        list_pack = list(
                            zip(self.dialogues_ids_list[i], self.dialogues_sent_len_list[i], self.role_list[i], self.handoff_list[i], self.senti_list[i]))
                        random.shuffle(list_pack)
                        self.dialogues_ids_list[i], self.dialogues_sent_len_list[i], self.role_list[i], self.handoff_list[i], self.senti_list[i] = zip(*list_pack)
    ...
    def data_generator(self, mode='train', batch_size=32, shuffle=True, nb_classes=3, epoch=0):
        ...
        if shuffle:
            ...
            random.seed(epoch + 7)
            random.shuffle(list_pack)
            dialog_ids_list[:], sent_len_list[:], dial_len_list[:], sess_id_list[:], \
                role_list[:], handoff_list[:], senti_list[:], score_list[:] = zip(*list_pack)
...
if __name__ == '__main__':
    ...
    if args.phase == 'test_load':
        ...
        for batch_dialog_ids, batch_sent_len, batch_dia_len, batch_ids, batch_role_ids, \
            batch_handoff, batch_senti, batch_score in data_loader.data_generator(mode=args.mode):
